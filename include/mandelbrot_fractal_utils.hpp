...
CalculateIterationsForPoint(const Complex &c,
...
[[nodiscard]]
constexpr Complex Pixel2DToComplex(std::uint32_t x,
                                   std::uint32_t y,
...
{
  const double real =
    viewport.x_min + (double(x) / screen_width) * viewport.width();
  const double imag =
    viewport.y_min + (double(y) / screen_height) * viewport.height();

  return Complex{real, imag};
}

[[nodiscard]]
constexpr RgbColor IterationsToColor(std::uint32_t iterations,
